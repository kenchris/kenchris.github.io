...
var fbird = (function () {
  ...
  var birds = (function () {
    ...
    var buffer = new ArrayBuffer(2097152);
    ...
    function init(maxPosition) {
      ...
      var accelDataValues = [10, 9.5, 9, 8, 7, 6, 5.5, 5, 5, 5, 5.5, 6, 7, 8, 9, 10].map(function (v) {
        return 50 * v;
      });
      accelDataValuesLength = accelDataValues.length;
      for (i = 0; i < accelDataValuesLength; i++) bufferF32[i + config.maxBirds * 2] = accelDataValues[i];
    }
    ...
    function fbirdModule(global, imp, buffer) {
      ...
      function updateAll(timeDelta) {
        ...
        //      var steps               = Math.ceil(timeDelta/accelData.interval);
        var steps = 0;
        var subTimeDelta = toF(0);
        var subTimeDeltaSquared = toF(0);
        var i = 0;
        var accelIndex = 0;
        var newPos = toF(0);
        var newVel = toF(0);
        ...
        subTimeDelta = toF(toF(timeDelta / toF(steps | 0)) / toF(1000));
    ...
    function updateAllSimd(timeDelta) {
      ...
      for (var i = 0, len = actualBirds + 3 >> 2; i < len; ++i) {
        var accelIndex = 0;
        var newVelTruex4;
        var newPosx4 = SIMD.float32x4(bufferF32[i * 4], bufferF32[i * 4 + 1], bufferF32[i * 4 + 2], bufferF32[i * 4 + 3]);
        var newVelx4 = SIMD.float32x4(bufferF32[i * 4 + maxBirds], bufferF32[i * 4 + maxBirds + 1], bufferF32[i * 4 + maxBirds + 2], bufferF32[i * 4 + maxBirds + 3]);
        for (var a = 0; a < steps; ++a) {
          var accel = bufferF32[accelIndex + config.maxBirds * 2];
    ...
    function fbirdSimdModule(global, imp, buffer) {
      ...
      var maxBirds = 100000;
      var maxBirdsx4 = 400000;
      var maxBirdsx4Plus4 = 400004;
      var maxBirdsx4Plus8 = 400008;
      var maxBirdsx4Plus12 = 400012;
      var maxBirdsx8 = 800000;
      var accelMask = 60;
      var mk2 = 1048572;
      ...
      function updateAllSimd(timeDelta) {
        ...
        var actualBirds = 0;
        var maxPos = toF(0);
        var maxPosx4 = f4(0, 0, 0, 0);
        var subTimeDeltax4 = f4(0, 0, 0, 0);
        var subTimeDeltaSquaredx4 = f4(0, 0, 0, 0);
        var point5x4 = f4(0.5, 0.5, 0.5, 0.5);
        var i = 0;
        var len = 0;
        var accelIndex = 0;
        var newPosx4 = f4(0, 0, 0, 0);
        var newVelx4 = f4(0, 0, 0, 0);
        var accel = toF(0);
        var accelx4 = f4(0, 0, 0, 0);
        var a = 0;
        var posDeltax4 = f4(0, 0, 0, 0);
        var cmpx4 = i4(0, 0, 0, 0);
        var newVelTruex4 = f4(0, 0, 0, 0);
        ...
        for (i = 0; (i | 0) < (len | 0); i = i + 16 | 0) {
          ...
          newPosx4 = f4(toF(f32[(i & mk2) >> 2]), toF(f32[(i & mk2) + 4 >> 2]), toF(f32[(i & mk2) + 8 >> 2]), toF(f32[(i & mk2) + 12 >> 2]));
          newVelx4 = f4(toF(f32[(i & mk2) + maxBirdsx4 >> 2]), toF(f32[(i & mk2) + maxBirdsx4Plus4 >> 2]), toF(f32[(i & mk2) + maxBirdsx4Plus8 >> 2]), toF(f32[(i & mk2) + maxBirdsx4Plus12 >> 2]));
          for (a = 0; (a | 0) < (steps | 0); a = a + 1 | 0) {
            accel = toF(f32[(accelIndex & accelMask) + maxBirdsx8 >> 2]);
