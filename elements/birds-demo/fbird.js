...
var fbird = (function () {
  ...
  function BirdTracker() {
    ...
    var accelData = {
      ...
      values: [10, 9, 8, 7, 6, 5, 6, 7, 8, 9, 10].map(function (v) {
        return 50 * v;
      }),
      valueConst: 1000
    };
    ...
    function init(maxPosition) {
      actualBirds = 0;
      maxPos = maxPosition;
      if (params.accelSteps > 0) accelData.steps = params.accelSteps;
    }
    ...
    function updateAll(timeDelta) {
      ...
      var subTimeDelta = timeDelta / steps / 1000;
  ...
  function Surface() {
    ...
    function createImageSprite(imageSrc, width, height, scale) {
      ...
      if (scale != 1) img.style.transform = "scale(" + scale + ")";
    ...
    function createSprite(width, height, rgbaData) {
      if (useCanvas) return createCanvasSprite(width, height, rgbaData);
    ...
    function placeSprite(spriteId, x, y) {
      if (useCanvas) return placeCanvasSprite(spriteId, x, y);else return placeDOMSprite(spriteId, x, y);
    }
    ...
    function moveSprite(posId, x, y) {
      if (useCanvas) moveCanvasSprite(posId, x, y);else moveDOMSprite(posId, x, y);
    }
    ...
    function removeLastSprite() {
      if (useCanvas) removeLastCanvasSprite();else removeLastDOMSprite();
    }
  ...
  function FPSCounter() {
    ...
    function adjustBirds(time, birdCount, bird, addBirds, removeBirds) {
      ...
      if (frameCount === 0) {
        ...
        var fps = 1000 * frameCountMax / delta;
        currentFpsValue = fps;
        if (fps > targetFpsMax) {
  ...
  function Animator() {
    ...
    this.getStartValue = function (start, max, randomFunc) {
      if (start === "random") return randomFunc(max);

      if (start === "center") return Math.floor(max / 2);
    ...
    this.addBirds = function (bird, count) {
      for (var i = 0; i < count; ++i) this.addBird(bird);
    };

    this.removeBirds = function (count) {
      for (var i = 0; i < count; ++i) this.removeLastBird();
    };

    this.removeAllBirds = function () {
      while (this.allBirds.length > 0) this.removeLastBird();
    };
    ...
    this.setUseSIMD = function (use) {
      this.useSIMD = use;
      this.removeAllBirds();
      if (!this.useSIMD) this.birdSprite = this.birdSpriteBase;else this.birdSprite = this.birdSpriteSIMD;
    };
    ...
    this.moveAll = function (time) {
      ...
      if (params.adjustBirds) {
        if (fpsAccounting.adjustBirds(time, this.allBirds.length, this.birdSprite, this.addBirds.bind(this), this.removeBirds.bind(this))) params.self.$.birds.innerText = this.allBirds.length;
  ...
  function URLParamsParser(defaults) {
    ...
    function setValue(key, value) {
      if (typeof value === "string") {
        if (typeof parameters[key] === "boolean") value = value === "true";else if (typeof parameters[key] === "number") {
          if (Number.isInteger(parameters[key])) value = parseInt(value);else value = parseFloat(value);
        }
      }
  ...
  function close() {
    if (initialized) animator.close();
    initialized = false;
  }

  function start() {
    if (initialized) animator.start();
  }

  function stop() {
    if (initialized) animator.stop();
  }
